...
object OlympicDataAnalysis {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val swimWiseMedal = spark
      .sql(
        "select country,sport,sum(total_medals) from global_temp.olympic where sport='Swimming' group by sport,country ")
      .show()

    // Find the total number of medals tht India won year wise.
    val IndiaWon = spark
      .sql(
        "select country,year,sum(total_medals) from global_temp.olympic where country='India' group by country,year order by year desc")
      .show()

    // Find the total numbers of medals won by each country
    val countryWiseMedal =
      spark.sql("select country,sum(total_medals) from global_temp.olympic group by country").show()

    // Fin the total medal won by Pakistan
    val panWonMedal = spark
      .sql("select country,sum(total_medals) from global_temp.olympic where country='Indonesia' group by country")
      .show()
