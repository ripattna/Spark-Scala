...
object DataFrame_UseCase extends App {
  ...
  val df_hd = df_emp.withColumn(
    "DeptHeadCount",
    when(col("Dept") === "X",
         df_emp.where("Dept = 'X'").groupBy("Dept").count().select(col("count").cast("string")).head.getString(0))
      when (col("Dept") === "Y", df_emp
        .where("Dept = 'Y'")
        .groupBy("Dept")
        .count()
        .select(col("count").cast("string"))
        .head
        .getString(0))
  )
