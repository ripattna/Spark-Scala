...
object UnionOperation {
  ...
  def main(args:  Array[String]): Unit = {
    ...
    // Create SparkSession
    val spark: SparkSession = SparkSession.builder().master("local[1]").appName("UnionOperation").getOrCreate()

    // Creating log level
    spark.sparkContext.setLogLevel("WARN")

    // Step-1: Reading the first dataset
    val northDF = spark.read.format("csv")
      .option("sep", ",")
    ...
    val westDF = spark.read.format("csv")
      .option("sep", ",")
